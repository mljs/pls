...
import rawData from '../../data/simpleDataset.json' with { type: 'json' };
...
const numericValues = newM.get('iris', { format: 'matrix' }).values;
...
test('opls-nipals with iris multi Y', () => {
  let x = new Matrix(iris);
  ...
  ).toBeDeepCloseTo([-9.273996, -8.626531, -9.069555, -9.160778, -8.539729], 6);
  ...
  // [4,] -0.0695591173
  // [5,] -0.0747948970
  expect(model.scoresXOrtho.getColumn(0).slice().splice(0, 5)).toBeDeepCloseTo(
